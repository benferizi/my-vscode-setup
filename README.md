...
## Use Copilot from the BROWSER (desktop app not working? no problem)

If the desktop Copilot app keeps failing on your machine (frozen window, F11/fullscreen problems, GPU issues), you don't need it at all — everything works in a normal web browser:

```bash
python setup.py copilot-web
```

Your three browser options (all activate Copilot Pro+ with your GitHub sign-in):

1. **GitHub Codespaces (recommended)** — full VS Code in the browser: open <https://github.com/codespaces>, create a codespace on your repo. This repository ships `.devcontainer/devcontainer.json`, so **all recommended extensions (including Copilot + Copilot Chat) install automatically** — nothing local to break, and it survives any PC format.
2. **vscode.dev tunnel** — browser VS Code connected to your own machine's files: `python setup.py copilot-web --tunnel` (needs the `code` CLI installed).
3. **Zero-setup** — Copilot Chat at <https://github.com/copilot>, quick repo editing at `github.dev` (press `.` on any repo page), or <https://vscode.dev>.

It also installs a **`copilot` terminal command**: just type `copilot` in any terminal and Copilot Chat opens in your default browser (`copilot https://vscode.dev` opens any other page).
...
| `python setup.py copilot-app [--file <path>]` | Install the GitHub Copilot AppImage properly: auto-finds the download (or use `--file`), installs to `~/Applications`, makes it executable, adds a `github-copilot` launcher command in `~/.local/bin` (disables GPU compositing and uses X11 on Wayland to fix frozen/partially-rendered windows; supports `--fullscreen` / `--maximized` to start full screen or maximized — F11 toggles fullscreen; on Ubuntu 24.04+ also auto-applies `--no-sandbox` when AppArmor blocks user namespaces and falls back to `--appimage-extract-and-run` when FUSE 2 is missing; still broken? quit fully with `pkill -f GitHub-Copilot.AppImage` then run `github-copilot --safe` for software rendering), creates an app-menu desktop entry, checks FUSE (suggests `libfuse2t64` on Ubuntu 24.04+), and verifies the Copilot VS Code extensions |
| `python setup.py copilot-web [--tunnel]` | **Desktop app still broken? Use Copilot fully in the BROWSER** — no AppImage, no GPU/F11 problems: installs a `copilot` terminal command that opens Copilot Chat (`https://github.com/copilot`) in your browser (`copilot <url>` opens any other page), shows how to open full VS Code + Copilot in the browser via GitHub Codespaces (this repo ships `.devcontainer/devcontainer.json` so ALL recommended extensions install automatically), sets up the vscode.dev tunnel to this machine (with `--tunnel`), lists zero-setup entry points (github.com/copilot chat, github.dev, vscode.dev) and verifies/installs all extensions locally |
