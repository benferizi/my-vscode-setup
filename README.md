...
| `python setup.py copilot-web [--tunnel]` | **Desktop app still broken? Use Copilot fully in the BROWSER** — no AppImage, no GPU/F11 problems: shows how to open full VS Code + Copilot in the browser via GitHub Codespaces (this repo ships `.devcontainer/devcontainer.json` so ALL recommended extensions install automatically), sets up the vscode.dev tunnel to this machine (with `--tunnel`), lists zero-setup entry points (github.com/copilot chat, github.dev, vscode.dev) and verifies/installs all extensions locally |
